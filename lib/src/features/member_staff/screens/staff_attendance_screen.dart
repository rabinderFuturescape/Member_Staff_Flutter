...
class _StaffAttendanceScreenState extends State<StaffAttendanceScreen> {
  final MemberStaffAttendanceApi _attendanceApi = MemberStaffAttendanceApi(
    apiClient: ApiClient(),
  );
  ...
  void _updateSelectedDayStaff() async {
    try {
      ...
      // For now, we'll use mock data if no attendance data exists for the selected day
  ...
  Future<void> _saveAttendance() async {
    ...
    try {
      ...
      final entries = _updatedAttendances.values
      ...
      if (entries.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please mark at least one attendance')),
        );
        setState(() {
          _isSaving = false;
        });
  ...
  Future<void> _takePicture(String staffId) async {
    ...
    if (image != null) {
      ...
      final photoUrl = 'https://example.com/photos/${DateTime.now().millisecondsSinceEpoch}.jpg';
